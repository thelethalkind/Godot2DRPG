...
#	var dist = position.direction_to(target.position)
#
#	if dist.x > attackDist or dist.y > attackDist and dist.x < chaseDist or dist.y < chaseDist:
#		var EnemyVel = (target.position - position).normalized()
#
#		move_and_slide(EnemyVel * moveSpeed)
